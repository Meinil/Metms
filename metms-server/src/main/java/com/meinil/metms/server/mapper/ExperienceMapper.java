...
public interface ExperienceMapper extends BaseMapper<Experience> {
    ...
    @Select("SELECT distinct tb_experience.id, tb_experience.content, tb_experience.level, tb_experience.finished, name title, username stuName, created_time, correct FROM tb_experience INNER JOIN tb_user ON tb_user.account=tb_experience.stu_id INNER JOIN tb_plan ON tb_plan.id=tb_experience.plan_id INNER JOIN tb_connect ON tb_connect.teacher_id=#{account}")
